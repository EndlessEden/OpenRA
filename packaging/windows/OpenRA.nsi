...
Function .onInit
	!ifndef USE_PROGRAMFILES32
		SetRegView 64
	!endif
	ReadRegStr $INSTDIR HKLM "Software\OpenRA-Kang${SUFFIX}" "InstallDir"
	StrCmp $INSTDIR "" unset done
	unset:
	!ifndef USE_PROGRAMFILES32
		StrCpy $INSTDIR "$PROGRAMFILES64\OpenRA-Kang${SUFFIX}"
	!else
		StrCpy $INSTDIR "$PROGRAMFILES32\OpenRA-Kang${SUFFIX}"
	!endif
	done:
FunctionEnd
...
!define MUI_STARTMENUPAGE_REGISTRY_KEY "Software\OpenRA-Kang${SUFFIX}"
!define MUI_STARTMENUPAGE_REGISTRY_VALUENAME "Start Menu Folder"
!define MUI_STARTMENUPAGE_DEFAULTFOLDER "OpenRA-Kang"
...
Section "-Reg" Reg
	...
	WriteRegStr HKLM "Software\OpenRA-Kang${SUFFIX}" "InstallDir" $INSTDIR
	
	; Join server URL Scheme
	WriteRegStr HKLM "Software\Classes\openra-kang-ra-${TAG}" "" "URL:Join OpenRA server"
	WriteRegStr HKLM "Software\Classes\openra-kang-ra-${TAG}" "URL Protocol" ""
	WriteRegStr HKLM "Software\Classes\openra-kang-ra-${TAG}\DefaultIcon" "" "$INSTDIR\ra.ico,0"
	WriteRegStr HKLM "Software\Classes\openra-kang-ra-${TAG}\Shell\Open\Command" "" "$INSTDIR\RedAlert.exe Launch.URI=%1"
	...
	WriteRegStr HKLM "Software\Classes\openra-kang-cnc-${TAG}" "" "URL:Join OpenRA server"
	WriteRegStr HKLM "Software\Classes\openra-kang-cnc-${TAG}" "URL Protocol" ""
	WriteRegStr HKLM "Software\Classes\openra-kang-cnc-${TAG}\DefaultIcon" "" "$INSTDIR\cnc.ico,0"
	WriteRegStr HKLM "Software\Classes\openra-kang-cnc-${TAG}\Shell\Open\Command" "" "$INSTDIR\TiberianDawn.exe Launch.URI=%1"
	...
	WriteRegStr HKLM "Software\Classes\openra-kang-d2k-${TAG}" "" "URL:Join OpenRA server"
	WriteRegStr HKLM "Software\Classes\openra-kang-d2k-${TAG}" "URL Protocol" ""
	WriteRegStr HKLM "Software\Classes\openra-kang-d2k-${TAG}\DefaultIcon" "" "$INSTDIR\d2k.ico,0"
	WriteRegStr HKLM "Software\Classes\openra-kang-d2k-${TAG}\Shell\Open\Command" "" "$INSTDIR\Dune2000.exe Launch.URI=%1"
...
Section "Game" GAME
	...
	IntFmt $0 "0x%08X" $0
	WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "EstimatedSize" "$0"
...
Section "Desktop Shortcut" DESKTOPSHORTCUT
	SetOutPath "$INSTDIR"
	CreateShortCut "$DESKTOP\OpenRA-Kang - Red Alert${SUFFIX}.lnk" $INSTDIR\RedAlert.exe "" \
		"$INSTDIR\RedAlert.exe" "" "" "" ""
	CreateShortCut "$DESKTOP\OpenRA-Kang - Tiberian Dawn${SUFFIX}.lnk" $INSTDIR\TiberianDawn.exe "" \
		"$INSTDIR\TiberianDawn.exe" "" "" "" ""
	CreateShortCut "$DESKTOP\OpenRA-Kang - Dune 2000${SUFFIX}.lnk" $INSTDIR\Dune2000.exe "" \
...
Section "-Uninstaller"
	...
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "DisplayName" "OpenRA-Kang${SUFFIX}"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "UninstallString" "$INSTDIR\uninstaller.exe"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "QuietUninstallString" "$\"$INSTDIR\uninstall.exe$\" /S"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "InstallLocation" "$INSTDIR"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "DisplayIcon" "$INSTDIR\ra.ico"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "Publisher" "OpenRA developers"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "URLInfoAbout" "http://openra.net"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "Readme" "$INSTDIR\README.html"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "DisplayVersion" "${TAG}"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "NoModify" "1"
	WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}" "NoRepair" "1"
SectionEnd
...
Function ${UN}Clean
	...
	DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OpenRA-Kang${SUFFIX}"
	DeleteRegKey HKLM "Software\Classes\openra-kang-ra-${TAG}"
	DeleteRegKey HKLM "Software\Classes\openra-kang-cnc-${TAG}"
	DeleteRegKey HKLM "Software\Classes\openra-kang-d2k-${TAG}"
	...
	Delete "$DESKTOP\OpenRA-Kang - Red Alert${SUFFIX}.lnk"
	Delete "$DESKTOP\OpenRA-Kang - Tiberian Dawn${SUFFIX}.lnk"
	Delete "$DESKTOP\OpenRA-Kang - Dune 2000${SUFFIX}.lnk"
	DeleteRegKey HKLM "Software\OpenRA-Kang${SUFFIX}"
FunctionEnd
!macroend
...
;***************************
LangString DESC_GAME ${LANG_ENGLISH} "OpenRA engine, official mods and dependencies. Kang Version"
